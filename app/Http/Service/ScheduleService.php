...
class ScheduleService
{
    ...
    //合同搜索
    public function contract_search($data)
    {
        ...
        if($type=='no_update' && $keywords!='')
        {
            $created_at=json_decode($keywords);
            $standard=UserSchedule::where('created_at','>',$created_at[0])->where('created_at','<',$created_at[1])->groupBy('contract_id')->select('id')->get();
            foreach($standard as $item)
            {
                $contract_id[]=$item->id;
            }
            $data=$data->whereIn('sku',$contract_id);
        }
        return $data;
    }
    //合同排序
    public function contract_sort($data)
    {
